...
The agent uses Reinforcement Learning, more specifically a variant of the traditional Q-Learning algorithm, implementing Keras to train a deep neural network as the Q-Value function approximator, so as to optimize a policy in this environment (which is to say: to keep the pole upright for the longest amount of time, resulting in maximum reward). The general nature of this approach means that this agent could easily be adapted to other games and environments, which I intend to do in the future. 

I have also uploaded a presentation on Q-Learning in general and as applied to this problem, and how the use of deep learning is appropriate in allowing the Q-Learning approach to work in problem domains, such as this one, where the size of the action-state space renders the traditional method of Q-Tables unfeasible. 
